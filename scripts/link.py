#!/usr/bin/env python3
"""
link.py - Materialize a host manifest as symlinks in $HOME

The host manifest at hosts/$hostname/ is a literal $HOME mirror: an entry at
hosts/<host>/<relpath> becomes the symlink ~/<relpath> pointing at the entry's
resolved target. One identity rule, no special cases.

All created symlinks are tracked in ~/.config/dotfiles.json so re-runs can
remove stale links and update changed ones.
...
def walk(host_dir: Path, home: Path, system_root: Path) -> Dict[str, str]:
    """
    Walk the host manifest ($HOME mirror) and build a mapping of:
        home/<relpath> -> target_path

    For symlinks in host dir: resolve to absolute path (warn if outside
    system_root). For real files: use the host file path. For symlinked
    directories: leaf nodes — link the dir itself, don't recurse. For real
    directories: recurse.
    """
    # Files/dirs to ignore (system metadata, etc.)
    ...
    for root, dirs, files in os.walk(host_dir):
        ...
        for name in files:
            ...
            link_path = home / rel_from_host / name
            ...
            if item_path.is_symlink():
                ...
                symlinks[str(link_path)] = str(target)
            else:
                # Real file - link to it
                symlinks[str(link_path)] = str(item_path)
...
def main():
    ...
    host_dir = system_root / 'hosts' / hostname
    manifest_path = Path.home() / '.config' / 'dotfiles.json'

    print(f"System root: {system_root}")
    print(f"Hostname: {hostname}")
    print(f"Host manifest: {host_dir}")
    print()

    if not host_dir.exists():
        print(f"Error: Host directory {host_dir} does not exist.", file=sys.stderr)
        print("Create it with symlinks to dotfile sources first.", file=sys.stderr)
        sys.exit(1)

    # Load existing manifest
    old_manifest = load_manifest(manifest_path)

    # Walk the host $HOME mirror and build desired state
    new_manifest = walk(host_dir, Path.home(), system_root)
