;;; lisp/evil-lisp-state-setup.el -*- lexical-binding: t; -*-
;;; Commentary:
;; Doom adaptations for evil-lisp-state (fork: jaidetree/evil-lisp-state).
;; Called from the `use-package! evil-lisp-state' :config in config.el, so
;; it runs once evil and smartparens are loaded in an interactive session.
;; Kept in its own file (rather than inline in config.el) so that
;; evil-lisp-state-test.el can load and exercise this exact code in a
;; batch session, where Doom skips user config entirely.
;;; Code:

(defface +evil-lisp-state-modeline-face
...
(defvar-local +evil-lisp-state--modeline-cookie nil
...
(defun +evil-lisp-state-modeline-enter ()
  "Swap in `+evil-lisp-state-modeline-face' for the modeline evil indicator.
...
current, works around that without touching doom-modeline's code."
  (setq +evil-lisp-state--modeline-cookie
        (face-remap-add-relative 'doom-modeline-evil-user-state
                                  '+evil-lisp-state-modeline-face)))

(defun +evil-lisp-state-modeline-exit ()
  "Undo `+evil-lisp-state-modeline-enter', restoring the shared fallback face."
  (when +evil-lisp-state--modeline-cookie
    (face-remap-remove-relative +evil-lisp-state--modeline-cookie)
    (setq +evil-lisp-state--modeline-cookie nil)))

(defun +evil-lisp-state-setup ()
...
state; \"SPC k .\" toggles the state explicitly."
  ...
  ;; the fork's source (lisp/evil-lisp-state/evil-lisp-state.el) — nothing
  ;; to adapt on the config side for those.
  ...
  ;; `+evil-lisp-state-modeline-enter' for why this can't be done by
  ;; configuring doom-modeline directly.
  (add-hook 'evil-lisp-state-entry-hook #'+evil-lisp-state-modeline-enter)
  (add-hook 'evil-lisp-state-exit-hook #'+evil-lisp-state-modeline-exit))

(provide 'evil-lisp-state-setup)
